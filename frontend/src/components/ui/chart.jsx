        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={item.dataKey}
              className={cn(
                "flex w-full flex-wrap items-stretch gap-2 [&>svg]:text-muted-foreground [&>svg]:h-2.5 [&>svg]:w-2.5",
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              "flex items-center gap-1.5 [&>svg]:text-muted-foreground [&>svg]:h-3 [&>svg]:w-3"
            )}
          >
